var controller = {
        ...
        thisController.$this.find("#cmdTransferir").click(function () {           
            ...
            if (bandera_ticket)
            {
                ...
                httpSendCallIVRDCP(postData, function (resp)
                {
                    ...
                    var callId = cicGetCallId();
                    
                    var postData = {
                        "ID_llamada" : callId.toString(),
                        "num_cliente" : telefono_DCP.toString(),
                        "ticket" : amountObject.ticket,
                        "estatus" : resp.statusCode,
                        "descripcion" : resp.statusDescription
                    };

                    httpReporteIVRDCP(postData, function (resp)
                    {
                        if (resp.status === "OK")
                        {
                            cicTrace("[Ticket_DCP] :: [httpReporteIVRDCP] :: [Exito] :: reporte generado ");
                        }
                        else if(resp.status == "NOK")
                        {
                           cicTrace("[Ticket_DCP] :: [httpReporteIVRDCP] :: [Exito] :: No se logro generar el reporte. " + resp.detalle);
                        }
                    },function (error)
                    {
                        cicTrace("[Ticket_DCP] :: [httpReporteIVRDCP] :: No se logro generar el reporte. " + error.detalle);
                    });
            ...
            if (typeTransfer.toLowerCase() == "ciega")
			{
				...
				if(key == "600188")
				{
					/*thisController.$this.find("#edSpeedDial option[value='600188']").css('display','none');
					thisController.$this.find("#edSpeedDial").children("option[value='600188']").css('display','none');
					document.getElementById("edSpeedDial").options[2].hidden = true;*/
					//thisController.$this.find("#edSpeedDial").children("option[value='600188']").hide();
					thisController.$this.find('#edSpeedDial option[value="600188"]').attr('disabled', 'disabled').hide();
				}
				//thisController.$this.find("#edSpeedDial").children("option[value='600188']").hide();
			...
			else if (typeTransfer.toLowerCase() == "ivr")
			{
                ...
                if (thisController.IVRConfirmation) {
                    if (thisController.IVRConfirmationCount > thisController.IVRMaximoEjecucionesConfirmation) {
                        showMsgBoxAlert("Se ha superado el mÃ¡ximo de confirmaciones permitidas para una gestiÃ³n.", "ConfirmaciÃ³n de Venta");
                        return;
                    }
                }

                showMsgBoxConfirm("Â¿Esta seguro que desea ejecutar la ConfirmaciÃ³n de Venta?", "ConfirmaciÃ³n de Venta", function (result) {
                    if (result == "Si") {
                        ...
                        if (response.status) {
                            ...
                            $.blockUI({ message: '<h2><img src="img/busy.gif" /> IVR de ConfirmaciÃ³n en curso. <br>Favor espere</h2>', css: { backgroundColor: '#004262', color: '#fff' } });
            ...
            if (campana.estado == "call") {
                if (thisController.lastStatusAgent.toUpperCase() == "CAMPAIGN CALL" || thisController.lastStatusAgent.toUpperCase() == "LLAMADA DE CAMPAÑA") {
                    if (estadoAgente.toUpperCase() == "FOLLOW UP" || estadoAgente.toUpperCase() == "SEGUIMIENTO") {
                        cicTrace("Se inicia Timer para cierre automatico de llamada: " + thisController.cierreAutomaticoLlamada + " minutos");
    ...
    alertAutomaticFinish: function () {
        ...
        showMsgBoxAlert("En " + delta + " minutos se realizará el cierre automático de la llamada", "Cierre Automático");
    },
    ...
    hacerRediscado: function (i3Identity) {
        ...
        var numero =  (metaDataObject ? metaDataObject.phoneNumber : cicGetNumberToDial());
    ...
    finalizadaLlamadaIVRConfirmacion: function (type) {
        ...
        var msgDesconexionAgente = "Estimado ejecutivo se ha desconectado su estaciÃ³n. Favor verificar correcta conexiÃ³n de Ã©sta.";
        ...
        if (type == "CLIENT_DISCONNECT") {
            ...
            msg = "Se ha desconectado el cliente durante la validaciÃ³n de ConfirmaciÃ³n de Venta.";
            ...
            if (thisController.agentDisconnectConfirmation) showMsgBoxAlert(msgDesconexionAgente, "ConfirmaciÃ³n de Venta");
            msg = "ConfirmaciÃ³n de Venta Finalizada.";
            ...
            if (thisController.agentDisconnectConfirmation) showMsgBoxAlert(msgDesconexionAgente, "ConfirmaciÃ³n de Venta");
            msg = "Se ha desconectado el IVR de ConfirmaciÃ³n de Venta.";
            showMsgBoxAlert(msg, "ConfirmaciÃ³n de Venta");
            cicPickup(thisController.CallIDdeConferenciaMUTE);
            cicMute(thisController.CallIDdeConferenciaMUTE);
        } else {
            ...
            if (thisController.agentDisconnectConfirmation) showMsgBoxAlert(msgDesconexionAgente, "ConfirmaciÃ³n de Venta");
            msg = "Ha ocurrido un error y se ha interrumpido la ConfirmaciÃ³n de Venta.";
            showMsgBoxAlert(msg, "ConfirmaciÃ³n de Venta");
            cicPickup(thisController.CallIDdeConferenciaMUTE);
            cicMute(thisController.CallIDdeConferenciaMUTE);
        }
        ...
        cicTrace("ConfirmaciÃ³n de Venta Finalizada con " + thisController.IVRConfirmation + ". Msg: " + msg);
    },
