        ...
        thisController.$this.find(".ticketV").blur(function ()
        {
			...
			httpVaidaTicket(ticketAValidar, function (resp)
			{
				...
				var postData = {
					...
					"ticket" : ticketAValidar,
					"estatus" : resp.statusCode,
					"descripcion" : resp.statusDescription
				};

				httpReporte_valida_ticket_IVRDCP(postData, function (resp)
				{
					if (resp.status === "OK")
					{
						cicTrace("[Ticket_DCP] :: [httpReporte_valida_ticket_IVRDCP] :: [Exito] :: reporte generado ");
					}
					else if(resp.status == "NOK")
					{
					   cicTrace("[Ticket_DCP] :: [httpReporte_valida_ticket_IVRDCP] :: [Exito] :: No se logro generar el reporte. " + resp.detalle);
					}
				},function (error)
				{
					cicTrace("[Ticket_DCP] :: [httpReporte_valida_ticket_IVRDCP] :: No se logro generar el reporte. " + error.detalle);
				});
			...
			},function (error)
			{
				...
				var postData = {
					...
					"num_cliente" : telefono_DCP.toString(),
					"ticket" : ticketAValidar,
					"estatus" : error.statusCode,
					"descripcion" : error.statusDescription
				};
