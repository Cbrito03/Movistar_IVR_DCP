    ...
    init: function (container, options) {
        ...
        thisController.$this.find("#cmdTransferir").click(function () {
          ...
          if (typeTransfer.toLowerCase() == "ciega"  || typeTransfer.toLowerCase() == "asistida") {
			              cicTrace("Inicia transferencia ciega" );
                            ...
                            if (thisController.globalConferenceObject != null) {
                                ...
                                thisController.IVRCallObject.stateChangeHandler = function (StateID, StateString) {
                                    ...
                                    if (StateID == "106" && thisController.IVRConfirmation == "PROCESSING") {
                                        ...
                                        if (StateString == "Desconectado [Desconexión remota]" || StateString == "Disconnected [Remote Disconnect]") {
...
enableButton(thisController.$this.find("#cmdRediscar"), true);
    ...
    refreshEstadoFinalizacion: function () {
       ...
       // var familia = (metaDataObject ? metaDataObject.callFamiliaId : cicGetCallFamiliaId());
        ...
        if (campana.estado == "call") {        
            ...
            //Valida NO VENTA - 9
            var result = $.grep(wrapUpCodesForFamilia, function (wrapUpCode) {
                return (wrapUpCode.ESTADO_FINALIZACION == "9");
            })

            if (result.length > 0) {
                var estado = new Object();
                estado.value = "9";
                estado.descripcion = "NO VENTA";
                estados.push(estado);
            }
    ...
    establecerConferencia: function () {
        ...
        try {
            ...
            response.msg = "Existe un error en comunicación con IVR. Favor contactar nuevamente.";
        ...
        try {
            if (thisController.globalConferenceObject == null) {
                ...
                cicTrace("Se ha creado Conferencia");
            }
        } catch (ex) {
            response.status = false;
            response.type = "conference";
            response.msg = "Ha ocurrido un problema al momento de organizar la confirmación. Favor intentar nuevamente.";
        ...
        try {
            if (!thisController.hasFirstCreateInConference) {
                ...
                cicTrace("Inicializa conferencia con Llamada a Destino");
            } else {
                thisController.globalConferenceObject.add(thisController.conferenceDestinyCallObject);
                cicTrace("Se agrega llamada de Destino a Conferencia");
            }
        } catch (ex) {
            ...
            response.msg = "Se ha perdido comunicación con IVR. Favor contactar nuevamente.";
        ...
        try {
            ...
            response.msg = "Se ha perdido comunicación con Cliente. Favor contactar nuevamente y luego intentar establecer Confirmaación.";
