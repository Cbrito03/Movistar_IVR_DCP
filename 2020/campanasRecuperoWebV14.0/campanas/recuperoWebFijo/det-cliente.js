...
var controller = {
    ...
    enviarReferido: function () {
        ...
        if (!telefono) {
            ...
            showMsgBoxAlert("El Telefono debe contenter 9 Dígitos", "Validacón Telefono");
            return;
        }

        if (!nombre) {
            showMsgBoxAlert("Debe ingresar Nombre para ingresar un Cliente Referido ", "Validación Nombre");
            return;
        }
        if (observacion) {
            if (observacion.length > 200) {
                showMsgBoxAlert("Excedió largo permitido para la Observación", "Valdación Obseración");
                return;
            }
        }
    ...
    calculaEdadCliente: function (esRespaldo) {
        ...
        if (thisController.isDate(txtVal)) {
            ...
            showMsgBoxAlert('Fecha No Válida, debe ingresar Fecha de Nacimiento correcta', "Valdiación Fecha");
            thisController.$this.find("#edEdad" + (esRespaldo ? "Resp" : "")).val("");
            return;
        }
