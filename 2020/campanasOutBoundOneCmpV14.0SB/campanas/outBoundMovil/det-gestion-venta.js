var controller = {
    ...
    init: function (container, options) {
        ...
        cicTrace("Gestion-Venta - Inicio Init");
            ...
            if (selected.val() != "Seleccione") {
                ...
                $.each(equipos, function (i, equipo) {
                    var planes = $.grep(thisController.infoPlanes, function (plan) {
                     ...
                     $.each(planes, function (i, planToAdd) {
                        var planesAdd = $.grep(listPlanes, function (plan) {
                            return ((plan.COD_PLAN + "|" + plan.PLAN) == (planToAdd.COD_PLAN + "|" + planToAdd.PLAN));
                        });
                        if (planesAdd.length == 0) listPlanes.push(planToAdd);
                    })
                })
            } else {
        ...
        thisController.$this.find("#edModoPago").change(function (event) {
            ...
            if (modoPago == 3301) {
                ...
                thisController.$this.find('#edCuotas').val(35);
            } else {
        ...
        thisController.$this.find("#edTipoVenta").change(function (event) {
            var equipoSelected = thisController.$this.find("#edTipoVenta").val();
            if (equipoSelected == 3801) {
        ...
        thisController.$this.find(".telefono").blur(function () {
            ...
            if (!$(this).inputmask("isComplete")) {
                showMsgBoxAlert("El telefono debe contener 8 o 9 dí­gitos","Validacion telefono");
                $(this).val("");
            }
        });

        cicTrace("Gestion-Venta - Fin Init");
    },
    refresca: function () {
        var thisController = this;
        cicTrace("gestion-venta Inicio Refresca");
        ...
        cicTrace("gestion-venta Fin Refresca");
    ...
    agregarProductos: function () {
        ...
        if (thisController.$this.find("#edCodigoPlan").val() == "Seleccione") {
            showMsgBoxAlert("Debe seleccionar Plan","Seleccion Plan");
            return;
        }
        
        if (thisController.$this.find("#edMarca").val() == "Seleccione" || thisController.$this.find("#edModeloEquipo").val() == "Seleccione") {
            showMsgBoxAlert("Debe seleccionar Marca y Modelo","Seleccionar Marca Modelo");
            return;
        }
        ...
        if (indexEquipo == -1) {
            showMsgBoxAlert("No se puede recuperar el identificador del producto","Idetificador");
            return;
        }
