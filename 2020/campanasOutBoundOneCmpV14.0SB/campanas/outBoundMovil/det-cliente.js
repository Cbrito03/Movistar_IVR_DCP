...
var controller = {
    ...
    init: function (container, options) {
        ...
        thisController.$this.find(".telefono").blur(function () {
            ...
            if (!$(this).inputmask("isComplete")) {
                showMsgBoxAlert("El telefono debe contener 9 dígitos", "Validación telefono");
            }
        });
    ...
    refresca: function () {
        ...
        var esAgendado = false;

        if (cicGetSbxReferido() == "A" || cicGetSbxConvergencia() == "A") esAgendado = true;
        ...
        if (esAgendado) {
            ...
            params.i3Identity = cicGetI3Identity();

            httpInvoke("getInfoExtraAgendado.ges", { param: params }, function (resp) {
                if (resp.foundData) {
                    thisController.$this.find("#edSerieRut").val(resp.serie);
                    thisController.$this.find("#edEmail").val(resp.email);
                    thisController.$this.find("#edEntreCalle").val(resp.entreCalle);
                    thisController.$this.find("#edYCalle").val(resp.yCalle);
                    if (resp.fechaNacimiento) {
                        thisController.$this.find("#edDiaNacimiento").val(resp.fechaNacimiento.dia);
                        thisController.$this.find("#edMesNacimiento").val(resp.fechaNacimiento.mes);
                        thisController.$this.find("#edAnoNacimiento").val(resp.fechaNacimiento.ano);
                    }
                }
    ...
    enviarReferido: function () {
        ...
        if (!telefono) {
            showMsgBoxAlert("Debe ingresar Fono para ingresar un Cliente Referido ", "Validación Referido");
            return;
        }else if (telefono.length < 9) {
            showMsgBoxAlert("El Telefono debe contenter 9 Dígitos", "Validación Telefono");
            return;
        }

        if (!nombre) {
            showMsgBoxAlert("Debe ingresar Nombre para ingresar un Cliente Referido ","Validacion Referido");
            return;
        }
        if (observacion) {
    ...
    calculaEdadCliente: function(esRespaldo){
        ...
        if (thisController.isDate(txtVal)) {
            if (!thisController.isMayorEdad(txtVal)) {
                showMsgBoxAlert("No se puede vender a Clientes menores de edad","Validación Cliente");
            ...
            showMsgBoxAlert('Fecha No Válida, debe ingresar Fecha de Nacimiento correcta',"Validación Fecha");
            thisController.$this.find("#edEdad" + (esRespaldo ? "Resp" : "")).val("");
            return;
        }
