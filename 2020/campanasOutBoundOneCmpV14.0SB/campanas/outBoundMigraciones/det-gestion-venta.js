var controller = {
    ...
    refresca: function (infoGestionVenta) {
        var thisController = this;
        ...
        if (!infoGestionVenta) {
            ...
            thisController.$this.find("#edPlan").val("Seleccione");
            thisController.$this.find("#edCodigoPlan").val("Seleccione");
            ...
            if (infoGestionVenta.listaProductosParrilla.length > 0) {
                $.each(infoGestionVenta.listaProductosParrilla, function (i, r) {
                    thisController.productos.push(r);
                    gridProductos.jqGrid("addRowData", r.id, r);
                });
            }

            thisController.$this.find("#edPlan").val("Seleccione");
            thisController.$this.find("#edCodigoPlan").val("Seleccione");
            thisController.$this.find("#edObservacionesVenta").val(infoGestionVenta.observaciones);
        }
    ...
    agregarProductos: function () {
        ...
        if (thisController.$this.find("#edPlan").val() == "Seleccione" || thisController.$this.find("#edCodigoPlan").val() == "Seleccione") {
            alert("Debe seleccionar Plan o Código de Plan");
            return;
        }
